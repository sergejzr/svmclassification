...
public class FeatureVector implements Serializable {
	...
	this("",factor,dims,vals);
    }
    public FeatureVector(String id,double factor, int[] dims, double[] vals) {
    	this.id=id;
...
this("",dims,vals);
    ...
    public FeatureVector(String id,int[] dims, double[] vals) {
    	this.id=id;
	this.m_factor = 1.0;
	this.m_dims = dims;
	this.m_vals = vals;
    ...
    public void addToBuffer(StringBuffer sb)
    {	
	for (int i = 0; i < m_vals.length; i++) {
		sb.append(m_dims[i]);
		sb.append(":");
		sb.append(m_vals[i]);
		if(i < m_vals.length - 1){sb.append(" ");}
	}
    }
    public void writeout(Writer w) throws IOException
    {
    	for (int i = 0; i < m_vals.length; i++) {
    		w.write(Integer.toString(m_dims[i]));
    		w.write(":");
    		w.write(Double.toString(m_vals[i]));
    		if(i < m_vals.length - 1){w.write(" ");}
    	}
    }
