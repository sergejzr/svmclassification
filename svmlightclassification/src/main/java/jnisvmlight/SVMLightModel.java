 ...
 */
public class SVMLightModel implements Model{
  ...
  /**
  * computes preference order of the dimensions
  * Stores it in the int- array "preferenceOrder"
  */
  public void computedDimOrder()
  {
    preferenceOrder = new int[m_linWeights.length];
    ArrayList<DimWeight> list = new ArrayList<DimWeight>(); 
    for (int i = 0; i < m_linWeights.length; i++)
    {
      list.add(new DimWeight(i,m_linWeights[i])); 
    }
    Collections.sort(list); 
    for (int i = 0; i < list.size(); i++)
    {
      int next = list.get(i).dim; 
      preferenceOrder[i] = next; 
    }
  }
  ...
  /**
  * Klassifikation mit reduziertem FeatureVector
  */
  public double classify(FeatureVector v, int reduction)
  {
    HashSet<Integer> dimSet = new HashSet<Integer>(); 
    for (int i = 0; i < reduction && i < preferenceOrder.length; i++)
    {
      dimSet.add(new Integer(preferenceOrder[i])); 
    }
    double delta = 0;
    if (m_kType == 0) {
      for (int i = 0; i < v.m_dims.length; i++)
      {
        if (dimSet.contains(v.m_dims[i]))
        {
          delta += v.m_factor * m_linWeights[v.m_dims[i]] * v.m_vals[i];
        }
      }
  ...
  public int getNormalVectorSize()
  {
    int size = 0; 
    for (int i = 0; i < m_linWeights.length; i++)
    {
...
public double getThreshold(){return m_threshold;}
  ...
  public void writeModelToFileFast(String path) {
	    File dump = new File(path);
	    if (m_docs != null) {
	      String header = m_format
	          + "\n"
